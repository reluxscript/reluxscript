...
function hasDynamicChildren(children) {
  return children.some(child => {
    ...
    if (t.isConditionalExpression(expr)) { console.error("CONDITIONAL:", expr.consequent.type, expr.alternate.type);
...
function generateJSXElement(node, component, indent) {
  ...
  for (const attr of attributes) {
    if (t.isJSXAttribute(attr)) {
      ...
      if (name.startsWith('on')) {
        ...
        // Regular prop
        const val = generateAttributeValue(value);
        props.push(`["${name}"] = ${val}`);
      }
    }
  }
  ...
  if (childrenCode.length === 0) {
    ...
    const childrenArray = childrenCode.map(c => c.code).join(',\n' + indentStr + '    ');
